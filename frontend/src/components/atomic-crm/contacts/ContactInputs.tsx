...
import { REFERRAL_TYPES, SERVICE_AREAS } from "../companies/referralData";
...
const ContactPositionInputs = () => {
  return (
    <div className="flex flex-col gap-4">
      <h6 className="text-lg font-semibold">Referral Context</h6>
      <SelectInput
        source="referral_type"
        label="Referral Type"
        helperText={false}
        choices={REFERRAL_TYPES.map((type) => ({ id: type, name: type }))}
      />
      <SelectInput
        source="referral_county"
        label="Service Area / County"
        helperText={false}
        choices={SERVICE_AREAS.map((area) => ({ id: area, name: area }))}
      />
    </div>
  );
};
